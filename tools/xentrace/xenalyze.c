...
void sched_process(struct pcpu_info *p)
{
    ...
    if(ri->evt.sub == 0xf) {
        ...
        case TRC_SCHED_CLASS_EVT(RTDS, 1): /* TICKLE           */
            ...
            break;
        case TRC_SCHED_CLASS_EVT(RTDS, 2): /* SCHEDULE        */
            if(opt.dump_all) {
                struct {
        ...
        case TRC_SCHED_CLASS_EVT(RTDS, 4): /* JOB_RELEASE */
            if(opt.dump_all) {
                ...
                printf(" %s rtds:job_release d%uv%u, deadline = %"PRIu64", "
                       "budget = %"PRIu64"\n", ri->dump_header,
                       r->domid, r->vcpuid, dl, bg);
            }
            break;
        case TRC_SCHED_CLASS_EVT(RTDS, 5): /* SCHED_TASKLET    */
            ...
            break;
        case TRC_SCHED_CLASS_EVT(RTDS, 6): /* DIS_RUNNING        */
            if(opt.dump_all) {
                struct {
                    unsigned int vcpuid:16, domid:16;
                    unsigned int cur_dl_lo, cur_dl_hi;
                ...
                printf(" %s rtds:dis_running d%uv%u, deadline = %"PRIu64", "
                       "budget = %"PRIu64"\n", ri->dump_header,
                       r->domid, r->vcpuid, dl, bg);
            }
            break;
        case TRC_SCHED_CLASS_EVT(RTDS, 7): /* DIS_NOT_RUNNING        */
            if(opt.dump_all) {
                struct {
                    unsigned int vcpuid:16, domid:16;
                } *r = (typeof(r))ri->d;
            ...
            break;
        case TRC_SCHED_CLASS_EVT(RTDS, 8): /* VCPU_ENALBE        */
            if(opt.dump_all) {
                struct {
                    unsigned int vcpuid:16, domid:16;
                } *r = (typeof(r))ri->d;
